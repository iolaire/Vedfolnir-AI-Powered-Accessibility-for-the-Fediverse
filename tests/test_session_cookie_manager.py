...
if __name__ == '__main__':
    unittest.main()        
        with self.app.test_request_context():
            ...
            cookies = response.headers.getlist('Set-Cookie')
            cookie_header = next(cookie for cookie in cookies if 'session_id' in cookie)
            self.assertIn('Secure', cookie_header)
    ...
    def test_get_session_id_from_cookie(self):
        ...
        with self.app.test_request_context():
            ...
            with patch('session_cookie_manager.request') as mock_request:
                mock_request.cookies.get.return_value = 'test_session_123'
                
                session_id = self.cookie_manager.get_session_id_from_cookie()
                
                self.assertEqual(session_id, 'test_session_123')
                mock_request.cookies.get.assert_called_with('test_session_id')
    
    def test_get_session_id_from_cookie_none(self):
        """Test getting session ID when no cookie exists"""
        with self.app.test_request_context():
            with patch('session_cookie_manager.request') as mock_request:
    ...
    def test_clear_session_cookie(self):
        ...
        with self.app.test_request_context():
            ...
            cookies = response.headers.getlist('Set-Cookie')
            cookie_header = next(cookie for cookie in cookies if 'test_session_id' in cookie)
    ...
    def test_validate_cookie_security_valid(self):
        """Test cookie security validation with valid settings"""
        manager = SessionCookieManager(max_age=3600, secure=True)
        
        is_valid = manager.validate_cookie_security()
        
        self.assertTrue(is_valid)
    
    def test_validate_cookie_security_too_short(self):
        """Test cookie security validation with too short max_age"""
        manager = SessionCookieManager(max_age=60)  # 1 minute
    ...
    def test_validate_cookie_security_too_long(self):
        """Test cookie security validation with too long max_age"""
        manager = SessionCookieManager(max_age=604800 + 1)  # More than 7 days
    ...
    def test_refresh_session_cookie(self):
        ...
        with self.app.test_request_context():
            ...
            # Should be same as setting cookie
            cookies = response.headers.getlist('Set-Cookie')
            self.assertTrue(any('test_session_id=test_session_123' in cookie for cookie in cookies))
    
    @patch('session_cookie_manager.logger')
    def test_set_session_cookie_error_handling(self, mock_logger):
        """Test error handling in set_session_cookie"""
        with self.app.test_request_context():
            response = Mock()
            response.set_cookie.side_effect = Exception("Cookie error")
    ...
    @patch('session_cookie_manager.logger')
    def test_get_session_id_error_handling(self, mock_logger):
        """Test error handling in get_session_id_from_cookie"""
        with patch('session_cookie_manager.request') as mock_request:
            mock_request.cookies.get.side_effect = Exception("Request error")
    ...
    @patch('session_cookie_manager.logger')
    def test_clear_session_cookie_error_handling(self, mock_logger):
        """Test error handling in clear_session_cookie"""
        with self.app.test_request_context():
            response = Mock()
            response.set_cookie.side_effect = Exception("Cookie error")
...
class TestCreateSessionCookieManager(unittest.TestCase):
    ...
    def test_create_with_defaults(self):
        ...
        manager = create_session_cookie_manager(app_config)
        
        self.assertEqual(manager.cookie_name, 'session_id')
        self.assertEqual(manager.max_age, 86400)  # Default 1 day
        self.assertTrue(manager.secure)
