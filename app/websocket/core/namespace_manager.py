...
from .auth_handler import WebSocketAuthHandler, AuthenticationResult, AuthenticationContext
...
class WebSocketNamespaceManager:
    ...
    def _handle_namespace_connect(self, namespace: str, auth: Optional[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if auth_result != AuthenticationResult.SUCCESS:
                ...
                # For development, allow rate-limited connections with minimal auth
                if auth_result == AuthenticationResult.RATE_LIMITED:
                    self.logger.info("Allowing rate-limited connection for development")
                    # Create minimal auth context for rate-limited users
                    from flask import session
                    user_id = session.get('user_id', 1)  # Default to admin user
                    auth_context = AuthenticationContext(
                        user_id=user_id,
                        username='admin',
                        role=UserRole.ADMIN,
                        session_id=session.get('session_id', 'dev-session'),
                        is_admin=True,
                        permissions=set()
                    )
                else:
            ...
            self.logger.info(f"🔌 Connection stored: {session_id} for user {auth_context.username}")
            self.logger.info(f"📊 Total connections: {len(self._connections)}")
    ...
    def _handle_namespace_disconnect(self, namespace: str) -> None:
        ...
        try:
            ...
            self.logger.info(f"🔌 Disconnect request for session: {session_id}")
            self.logger.info(f"📊 Known sessions: {list(self._connections.keys())}")
