...
from .config_manager import ConsolidatedWebSocketConfigManager as WebSocketConfigManager
...
class WebSocketFactory:
    ...
    def __init__(self, config_manager: WebSocketConfigManager,
                 db_manager=None, session_manager=None):
        """
    ...
    def create_socketio_instance(self, app: Flask) -> SocketIO:
        ...
        try:
            ...
            self._setup_cors_headers(app)
            self._handle_preflight_requests(app)
    ...
    def _get_unified_socketio_config(self) -> Dict[str, Any]:
        ...
        # Convert WebSocket config to SocketIO format
        base_config = {
            ...
            'logger': websocket_config.enable_debug,
            'engineio_logger': websocket_config.enable_debug
        }
        
        # Get CORS configuration from config manager
        cors_config = {
            'cors_allowed_origins': websocket_config.cors_origins,
            'cors_credentials': websocket_config.cors_credentials
        }
    ...
    def _setup_security_middleware(self, socketio: SocketIO) -> None:
        ...
        """
        try:
            # Security middleware integrated into consolidated security manager
            pass
    ...
    def _setup_default_connection_handlers(self, socketio: SocketIO, namespace: str) -> None:
        ...
        def handle_connect(auth=None):
            ...
            try:
                ...
                if origin:
                    is_valid, error_msg = self._validate_websocket_origin(origin, namespace)
                    if not is_valid:
    ...
    def get_factory_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'cors_debug_info': self._get_cors_debug_info(),
    ...
    def validate_factory_configuration(self) -> bool:
        ...
        try:
            ...
            websocket_config = self.config_manager.get_websocket_config()
            cors_origins = websocket_config.cors_origins
            if not cors_origins:
    ...
    def _setup_cors_headers(self, app):
        """Setup CORS headers for Flask app"""
        ...
        def after_request(response):
            origin = request.headers.get('Origin')
            if origin and origin in websocket_config.cors_origins:
                response.headers['Access-Control-Allow-Origin'] = origin
                response.headers['Access-Control-Allow-Credentials'] = 'true'
                response.headers['Access-Control-Allow-Methods'] = ', '.join(websocket_config.cors_methods)
                response.headers['Access-Control-Allow-Headers'] = ', '.join(websocket_config.cors_headers)
            return response
    
    def _handle_preflight_requests(self, app):
    ...
    def _validate_websocket_origin(self, origin, namespace):
        """Validate WebSocket origin"""
        websocket_config = self.config_manager.get_websocket_config()
        if origin in websocket_config.cors_origins:
            return True, None
        return False, f"Origin {origin} not allowed"
    
    def _get_cors_debug_info(self):
        """Get CORS debug information"""
        websocket_config = self.config_manager.get_websocket_config()
        return {
            'allowed_origins': websocket_config.cors_origins,
            'credentials_allowed': websocket_config.cors_credentials,
            'allowed_methods': websocket_config.cors_methods,
            'allowed_headers': websocket_config.cors_headers
        }
